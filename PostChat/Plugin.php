...
class PostChat_Plugin implements PluginInterface
{
    ...
    public static function config(Form $form)
    {
        ...
        $form->addInput(new Text('postSelector', NULL, 'article', _t('文章选择器'), _t('文章选择器，用于选择文章内容。如果没有正常显示摘要，你需要访问 https://postsummary.zhheo.com/theme/custom.html#%E8%8E%B7%E5%8F%96tianligpt-postselector 学习获取，也可以联系 zhheo@qq.com 发送你的网站地址后获取')));
        $form->addInput(new Text('title', NULL, '文章摘要', _t('摘要标题'), _t('摘要标题，用于显示在摘要顶部的自定义内容')));
        $form->addInput(new Text('postURL', NULL, '*/archives/*', _t('文章路由'), _t('在符合url条件的网页执行文章摘要功能，通常情况下，绝大多数Typecho网站使用默认配置即可，无需调整')));
    ...
    {
        $settings = Options::alloc()->plugin('PostChatWidget');
        ...
        if ($scriptUrl) {
            echo '<script>
                ...
                let tianliGPT_typingAnimate = ' . $settings->typingAnimate . ';
                var postChatConfig = {
